...
parser.add_argument('--waymo', dest='waymo', help='waymo od path', default="")
...
loss_type = cfg.DATA_PRESET.get('LOSS_TYPE', 'MSELoss')
...
def vis(writer, image, pose):
    if pose is not None:
        image = writer.vis_frame(image, pose, writer.opt, writer.vis_thres)
    return image
...
if __name__ == "__main__":
    ...
    if mode == 'webcam':
        ...
        det_loader = WaymoDetectionLoader(input_source, cfg, args)
        det_worker = det_loader.start()
    else:
    ...
    try:
        for i in im_names_desc:
            ...
            with torch.no_grad():
                ...
                # vis_image = vis_frame(orig_img, hm, args, vis_thres)
                # print("HEAT MAP : ", hm)
                # import cv2
                # vis_image = vis_kp(orig_img, hm)
                # cv2.imwrite("/lhome/rauysal/AlphaPose/results/vis/" + im_name + ".png", vis_image)
